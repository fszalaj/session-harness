...
class SpendLedger:
    def __init__(self, path=None, *, ledger=None):
        ...
        with self.ledger._connect() as db:
            db.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS money_daily_anchors (
                    scope TEXT NOT NULL, day TEXT NOT NULL, fingerprint TEXT NOT NULL,
                    allowance TEXT NOT NULL, PRIMARY KEY(scope,day));
            """)
            db.execute("BEGIN IMMEDIATE")
            columns = {row[1] for row in db.execute("PRAGMA table_info(money_daily_anchors)")}
            for name in ("basis", "base_cap"):
                if name not in columns:
                    db.execute(f"ALTER TABLE money_daily_anchors ADD COLUMN {name} TEXT")
    ...
    def configure(self, scope, monthly, code="USD", mode="strict"):
        ...
        with self.ledger._connect() as db:
            ...
            previous = db.execute("SELECT currency,cap,mode FROM money_caps WHERE scope=?", (scope,)).fetchone()
    ...
    def _daily(self, db, caps, instant, *, persist=False):
        zone = ZoneInfo(self.ledger.policy["timezone"])
        today = datetime.fromtimestamp(instant, zone).date()
        day = today.isoformat()
        schedule = budget_policy.calendar(self.ledger._budgets(db))["workdays"]
        remaining = monthrange(today.year, today.month)[1] - today.day + 1
        days = sum((today.weekday() + offset) % 7 in schedule for offset in range(remaining))
        scheduled = today.weekday() in schedule
        for cap in caps:
            scope = cap["scope"]
            rows = db.execute("SELECT reserved,charged,status,created FROM money_requests" +
                              (" WHERE scope=?" if scope != "total" else ""),
                              (scope,) if scope != "total" else ()).fetchall()
            debit = sum((charged if status == "SETTLED" else reserved)
                        for reserved, charged, status, created in rows
                        if datetime.fromtimestamp(created, zone).date() == today)
            fingerprint = hashlib.sha256(json.dumps(
                [cap["cap_ticks"], cap["currency"], cap["mode"], schedule],
                separators=(",", ":")).encode()).hexdigest()
            anchor = db.execute("SELECT fingerprint,allowance,basis,base_cap FROM money_daily_anchors WHERE scope=? AND day=?",
                                (scope, day)).fetchone()
            basis = int(anchor[2]) if anchor and anchor[2] is not None else (
                int(anchor[1]) * max(days, 1) if anchor else cap["available_ticks"] + debit)
            base_cap = int(anchor[3]) if anchor and anchor[3] is not None else cap["cap_ticks"]
            if anchor and anchor[0] == fingerprint:
                allowance = int(anchor[1])
                if allowance < 0:
                    raise ValueError("invalid daily monetary anchor")
            else:
                allowance = max(0, basis + cap["cap_ticks"] - base_cap) // days if scheduled and days else 0
                if persist:
                    db.execute("INSERT OR REPLACE INTO money_daily_anchors(scope,day,fingerprint,allowance,basis,base_cap) VALUES (?,?,?,?,?,?)",
                               (scope, day, fingerprint, str(allowance), str(basis), str(base_cap)))
                    self._audit(db, "daily_anchor", dict(scope=scope, day=day, allowance=allowance))
            available = min(cap["available_ticks"], max(0, allowance - debit)) if scheduled else 0
            cap.update(day=day, workdays_remaining=days, scheduled_workday=scheduled,
                       daily_allowance_ticks=allowance, daily_allowance=amount(allowance),
                       daily_debit_ticks=debit, daily_debit=amount(debit),
                       daily_available_ticks=available, daily_available=amount(available))
        return caps
    ...
    def status(self, now=None):
        ...
        with self.ledger._connect() as db:
            ...
            return dict(month=month, timezone=self.ledger.policy["timezone"],
                        caps=self._daily(db, self._status(db, month), instant),
    ...
    def authorize(self, scope, reserve, code, digest, request_id=None, now=None):
        ...
        with self.ledger._connect() as db:
            ...
            if scope.startswith("api:"):
                self.ledger._require_setup(db, "api", scope.removeprefix("api:"))
            else:
                raise ValueError("paid native extra dispatch is unsupported")
            instant = time.time() if now is None else now
            month = self.month(instant)
            caps = self._daily(db, self._status(db, month, scope), instant, persist=True)
            ...
            for cap in caps:
                ...
                if reservation > cap["daily_available_ticks"]:
                    raise ValueError("daily monetary cap reached")
