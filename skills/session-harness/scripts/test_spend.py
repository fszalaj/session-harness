...
JAN = datetime(2026, 1, 31, tzinfo=timezone.utc).timestamp()
FEB = datetime(2026, 2, 28, tzinfo=timezone.utc).timestamp()


def complete_setup(ledger):
    ledger.complete_setup(services=["codex", "claude", "antigravity"],
                          api_services=["openai", "anthropic", "gemini", "xai", "deepseek", "kimi", "zai", "openrouter"],
                          source="test")
...
class MoneyTests(unittest.TestCase):
    ...
    def test_scope_and_total_apply_together_and_extra_has_separate_scope(self):
        ...
        self.money.record_expense("extra:claude", "8", "USD", "receipt:extra", "extra", JAN)
        self.assertEqual(0, next(c for c in self.money.status(JAN)["caps"] if c["scope"] == "total")["available_ticks"])
    ...
    def test_month_boundary_uses_ledger_timezone(self):
        ...
        money.authorize("api:xai", "0.1", "USD", DIGEST, "edge", stamp)
