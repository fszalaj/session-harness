...
class ProviderTests(unittest.TestCase):
    ...
    def test_unbudgeted_transport_and_generate_cannot_dispatch(self):
        prepared = api.preflight('kimi', 'future-model', 'text', 100)
        with patch('api_providers.request_json') as request:
            with self.assertRaisesRegex(APIError, 'monetary_admission_required'):
                api.execute(prepared, timeout=12)
            with self.assertRaisesRegex(APIError, 'use_budgeted_api_execution'):
                api.generate('kimi', 'future-model', 'text', 100)
