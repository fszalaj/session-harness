...
def execute(prepared, timeout=120):
    """Caller must commit monetary admission before invoking this transport."""
...
def generate(service, model, prompt, max_output_tokens, effort=None, *, timeout=120):
    return execute(preflight(service, model, prompt, max_output_tokens, effort), timeout)
