...
class ProfileInstallationTests(unittest.TestCase):
    ...
    def test_reinstall_preserves_environment_setup_and_ledger_bytes(self):
        ...
        scripts = next(path for path in (root / "skills/session-harness/scripts",
                                         root / ".claude/skills/session-harness/scripts") if path.is_dir())
        sys.path.insert(0, str(scripts))
        try:
            from quota import Ledger
        finally:
            sys.path.pop(0)
        ledger_path = self.home / ".local/state/session-harness/quota/ledger.sqlite3"
        ...
        self.assertEqual(hashlib.sha256(ledger_path.read_bytes()).hexdigest(), before)
        status = Ledger(ledger_path).setup_status()
        self.assertTrue(status["complete"])
        self.assertEqual((status["services"], status["api_services"]), (["claude"], ["xai"]))
        self.assertEqual(Ledger(ledger_path).mode(), "observed")
    ...
    def test_shared_entrypoints_and_native_roles_keep_the_profile_contract(self):
        ...
        for role in claude_roles:
            with self.subTest(role=role.name):
                ...
                # Leaf roles may name the cheaper rolling alias; nothing pins a version ID.
                allowed = {"inherit"} if "verifier" in role.name else {"inherit", "sonnet"}
                self.assertIn(model, allowed, "Inherit or use a rolling alias instead of pinning a version")
                self.assertNotRegex(model, r"[0-9]", "Native roles must not pin model versions")
